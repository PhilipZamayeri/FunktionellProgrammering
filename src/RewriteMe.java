...
public class RewriteMe {
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        ...
        return questions.stream().flatMap(s -> s.getAllAnswers().stream())
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        ...
        return questions.stream().anyMatch(answer -> answer.getAllAnswers().contains(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
       ...
       return questions.stream().filter(s -> s.getAllAnswers().contains(answerCandidate)).collect(Collectors.toList()).size();
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        ...
        return questions.stream().collect(groupingBy(s -> s.getCategory(), mapping(s -> s.getQuestionString(), toList())));
    }
    ...
    public String getLongestLettercountAnwerInAGivenCategory(Category c)
    {
        ...
        return questions.stream().filter(s -> s.getCategory().equals(c))
                .flatMap(s -> s.getAllAnswers().stream())
